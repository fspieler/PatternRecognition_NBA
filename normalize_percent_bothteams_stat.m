function [std_devs, prev_games] = normalize_percent_bothteams_stat( raw_data, year, team_int, game_num, stat_int)
    ...
    for i=1:82
        if raw_data( year, opp_team_int, i, stat2int('date') ) <= date
            prev_games(i) = raw_data( year, opp_team_int, i,...
                opp_stat_label(stat_int) ) / ( raw_data( year, opp_team_int, i,...
    ...
    comb_stat = raw_data( year, team_int, game_num, stat_int) / ( raw_data( year, team_int, game_num, stat_int)... 
        + raw_data( year, team_int, game_num, opp_stat_label(stat_int)) );
